...
namespace Deveplex.Repositorys.SqlClient
{
    public abstract class SqlServerRepository : IRepository<SqlConnection, SqlTransaction>
    {
        ...
        public int ExecuteQuery(string sql, SqlTransaction transaction, object parameters)
        {
            return ExecuteQuery(sql, CommandType.Text, transaction, parameters);
        }

        public int ExecuteQuery(string sql, CommandType commandType, SqlTransaction transaction, object parameters)
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                SqlParameter[] sqlParameters = connection.AnonymousTypeToSqlParameter(parameters);

                return connection.ExecuteNonQuery(sql, commandType, transaction, sqlParameters);
            }
        }
        ...
        public Nullable<T> ExecuteScalar<T>(string sql, SqlTransaction transaction, object parameters) where T : struct
        {
            return ExecuteScalar<T>(sql, CommandType.Text, transaction, parameters);
        }

        public Nullable<T> ExecuteScalar<T>(string sql, CommandType commandType, SqlTransaction transaction, object parameters) where T : struct
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                SqlParameter[] sqlParameters = connection.AnonymousTypeToSqlParameter(parameters);

                return connection.ExecuteScalar<T>(sql, commandType, transaction, sqlParameters);
            }
        }
        ...
        public IEnumerable<T> ExecuteQuery<T>(string sql, CommandType commandType, object parameters) where T : class, new()
        {
            ...
            {
                SqlParameter[] sqlParameters = connection.AnonymousTypeToSqlParameter(parameters);
                DataSet dataset = connection.ExecuteQuery<DataSet>(sql, commandType, sqlParameters);
